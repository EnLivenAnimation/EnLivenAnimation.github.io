...
function readParents(atlas){
    ...
    for(i = 0 ; i < parents.length;i++){
        if(i!=1){
            ...
            /*for(j = 0; j < childNum[i];j++){
                var children = document.createElement("li");
                for(k = 0 ; k < array2.length;k++){
                    var start = array2[k].indexOf(".");
                    var middle = array2[k].indexOf("(");
                    if(array2[k].substring(0,start)==parents[i]){
                        children.innerHTML=array2[k].substring(start+1,middle);
                        //array2.splice(k,1);
                    }
                }
                childrens.appendChild(children);
